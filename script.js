...
// Notification Icon & Mobile Menu Toggle
const notificationIcon = document.getElementById('notification-icon');
const notificationCount = document.getElementById('notification-count');
const mobileMenuToggle = document.getElementById('mobile-menu-toggle');
const sidebar = document.getElementById('sidebar');
const sidebarNotificationPlaceholder = document.getElementById('sidebar-notification-placeholder');
...
function hideMessageModal() {
    ...
    // Clear alerts in memory for demo purposes after viewing
    // In a real app, you would mark them as 'read' in Firestore
    activeMaintenanceAlerts = [];
    scheduledAlerts = [];

    // Re-check and update notification count when modal is closed
    checkMaintenanceAlerts(); // This will re-populate if new alerts are due
    updateNotificationCount(); // This will update the badge
}
...
function startSession(user) {
    ...
    // Show fixed header icons after login
...
logoutButton.addEventListener("click", () => {
    ...
    // Hide fixed header icons on logout
...
document.querySelectorAll(".nav-menu a").forEach(link => {
    link.addEventListener("click", () => {
        ...
        if (window.innerWidth < 768) {
            sidebar.classList.add('mobile-hidden'); // Hide sidebar on mobile after selection
            sidebar.classList.remove('mobile-visible');
            // Restore fixed header icons visibility if they were hidden by menu open
            mobileMenuToggle.classList.remove('hidden-on-menu-open');
            // Move notification icon back to its fixed position after a short delay
            setTimeout(() => {
                // Ensure the placeholder still contains the notification icon before moving it
                // This prevents errors if the icon was already moved by another event (e.g., direct click on notification)
                if (sidebarNotificationPlaceholder.contains(notificationIcon)) {
                    document.body.appendChild(notificationIcon);
                    notificationIcon.style.position = 'fixed';
                    notificationIcon.style.top = '1rem';
                    notificationIcon.style.right = '1rem';
                    notificationIcon.style.margin = '';
                    notificationIcon.style.width = '';
                    notificationIcon.style.pointerEvents = 'auto'; // Make it clickable again
                }
...
mobileMenuToggle.addEventListener('click', (event) => {
    ...
    sidebar.classList.toggle('mobile-visible'); // Toggles visibility based on CSS transitions
    sidebar.classList.toggle('mobile-hidden'); // Toggles visibility based on CSS transitions

    if (sidebar.classList.contains('mobile-visible')) {
        // Hide mobile menu button
        mobileMenuToggle.classList.add('hidden-on-menu-open');
        // Move notification icon into sidebar
        sidebarNotificationPlaceholder.appendChild(notificationIcon);
        // Adjust styles for inside sidebar
        notificationIcon.style.position = 'static'; // Or relative, adjust as needed
        notificationIcon.style.top = '';
        notificationIcon.style.right = '';
        notificationIcon.style.margin = '1rem auto'; // Center it or adjust as needed
        notificationIcon.style.width = 'fit-content'; // Reset width
        notificationIcon.style.pointerEvents = 'auto'; // Make it clickable inside sidebar
    } else {
        // Show mobile menu button
        mobileMenuToggle.classList.remove('hidden-on-menu-open');
        // Move notification icon back to body after a short delay
        setTimeout(() => {
            // Check if the icon is still in the placeholder before attempting to move it
            if (sidebarNotificationPlaceholder.contains(notificationIcon)) {
                document.body.appendChild(notificationIcon);
                // Reset styles for fixed position
                notificationIcon.style.position = 'fixed';
                notificationIcon.style.top = '1rem';
                notificationIcon.style.right = '1rem';
                notificationIcon.style.margin = '';
...
document.addEventListener('click', (event) => {
    // Check if the sidebar is currently visible in mobile mode
    const isSidebarVisibleOnMobile = window.innerWidth < 768 && sidebar.classList.contains('mobile-visible');

    // If sidebar is visible and the click is outside the sidebar and not on the toggle button or notification icon
    if (isSidebarVisibleOnMobile && !sidebar.contains(event.target) && !mobileMenuToggle.contains(event.target) && !notificationIcon.contains(event.target)) {
        sidebar.classList.add('mobile-hidden');
        sidebar.classList.remove('mobile-visible');
        // Restore fixed header icons visibility if they were hidden by menu open
        mobileMenuToggle.classList.remove('hidden-on-menu-open');
        ...
        return `
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td class="py-3 px-4 whitespace-nowrap text-right text-sm font-medium">
                    <button class="icon-button text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-200 ml-2" onclick="editVehicle('${v.id}')" title="Editar" aria-label="Editar vehículo ${v.plate}"><span class="material-icons">edit</span></button>
                    <button class="icon-button delete-button text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-200 ml-2" onclick="deleteVehicle('${v.id}')" title="Eliminar" aria-label="Eliminar vehículo ${v.plate}"><span class="material-icons">delete</span></button>
                </td>
            </tr>
        `;
    }).join('');
    ...
    driversList.innerHTML = drivers.map(d => {
        ...
        return `
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td class="py-3 px-4 whitespace-nowrap text-right text-sm font-medium">
                    <button class="icon-button text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-200 ml-2" onclick="editDriver('${d.id}')" title="Editar" aria-label="Editar conductor ${d.name}"><span class="material-icons">edit</span></button>
                    <button class="icon-button delete-button text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-200 ml-2" onclick="deleteDriver('${d.id}')" title="Eliminar" aria-label="Eliminar conductor ${d.name}"><span class="material-icons">delete</span></button>
                    <select onchange="assignVehicleToDriver('${d.id}', this.value)" class="ml-2 p-1 border rounded-md dark:bg-gray-600 dark:border-gray-500 dark:text-white" aria-label="Asignar vehículo a ${d.name}">
    ...
    accountsList.innerHTML = accounts.map(a => {
        ...
        return `
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td class="py-3 px-4 whitespace-nowrap text-right text-sm font-medium">
                    ${a.status !== 'approved' ? `<button class="icon-button text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-200 ml-2" onclick="approveAccount('${a.id}')" title="Aprobar cuenta" aria-label="Aprobar cuenta ${a.period} de ${driverName}"><span class="material-icons">check_circle</span></button>` : ''}
                    ${a.deductions.some(d => d.status === 'pending') ? `<button class="icon-button text-purple-600 hover:text-purple-900 dark:text-purple-400 dark:hover:text-purple-200 ml-2" onclick="reviewDeductions('${a.id}')" title="Revisar deducciones" aria-label="Revisar deducciones de cuenta ${a.period} de ${driverName}"><span class="material-icons">visibility</span></button>` : ''}
                </td>
            </tr>
        `;
    }).join('');
...
function deleteVehicle(vehicleId) {
    showMessageModal(`
        ...
        <div class="flex justify-center gap-4 mt-4">
            <button class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700" onclick="confirmDeleteVehicle('${vehicleId}')" aria-label="Confirmar eliminación de vehículo">Sí, Eliminar</button>
            <button class="bg-gray-300 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-400" onclick="hideMessageModal()" aria-label="Cancelar eliminación de vehículo">Cancelar</button>
        </div>
    `);
}
...
function deleteDriver(driverId) {
    showMessageModal(`
        ...
        <div class="flex justify-center gap-4 mt-4">
            <button class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700" onclick="confirmDeleteDriver('${driverId}')" aria-label="Confirmar eliminación de conductor">Sí, Eliminar</button>
            <button class="bg-gray-300 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-400" onclick="hideMessageModal()" aria-label="Cancelar eliminación de conductor">Cancelar</button>
        </div>
    `);
}
...
function reviewDeductions(accountId) {
    ...
    let deductionListHtml = account.deductions.map((d, index) => `
        <div class="flex justify-between items-center p-2 border-b last:border-b-0 dark:border-gray-600">
            ...
            ${d.status === 'pending' ? `
                <div>
                    <button class="icon-button text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-200 mr-2" onclick="approveDeduction('${accountId}', ${index})" title="Aprobar Deducción" aria-label="Aprobar deducción ${d.description}"><span class="material-icons">check_circle</span></button>
                    <button class="icon-button text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-200" onclick="rejectDeduction('${accountId}', ${index})" title="Rechazar Deducción" aria-label="Rechazar deducción ${d.description}"><span class="material-icons">cancel</span></button>
                </div>
            ` : ''}
        </div>
    `).join('');
...
function renderScheduledAlerts() {
    ...
    scheduledAlerts.forEach(alert => {
        ...
        row.innerHTML = `
            ...
            <td class="py-3 px-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="icon-button delete-button text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-200 ml-2" onclick="deleteAlert('${alert.id}')" title="Eliminar" aria-label="Eliminar alerta"><span class="material-icons">delete</span></button>
                <button class="icon-button text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-200 ml-2" onclick="sendDesktopNotification('${alert.message}', 'Alerta de ${alert.type}')" title="Enviar Ahora" aria-label="Enviar alerta ahora"><span class="material-icons">send</span></button>
            </td>
        `;
...
async function deleteAlert(alertId) {
    showMessageModal(`
        ...
        <div class="flex justify-center gap-4 mt-4">
            <button class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700" onclick="confirmDeleteAlert('${alertId}')" aria-label="Confirmar eliminación de alerta">Sí, Eliminar</button>
            <button class="bg-gray-300 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-400" onclick="hideMessageModal()" aria-label="Cancelar eliminación de alerta">Cancelar</button>
        </div>
    `);
}
...
// Function to handle scroll visibility of header icons
function handleScrollVisibility() {
    const currentScrollY = window.scrollY;

    // Only apply scroll hiding if sidebar is NOT open on mobile
    if (!(window.innerWidth < 768 && sidebar.classList.contains('mobile-visible'))) {
        if (currentScrollY > lastScrollY && currentScrollY > 60) { // Scrolled down past a threshold
            if (!isScrollingDown) {
                document.body.classList.add('scrolled-down');
                isScrollingDown = true;
            }
        } else if (currentScrollY < lastScrollY || currentScrollY < 60) { // Scrolled up or back to top
            if (isScrollingDown) {
        ...
        // If sidebar is open, ensure scroll-down effect is removed
        document.body.classList.remove('scrolled-down');
        isScrollingDown = false;
    }
    ...
    // Debounce to prevent rapid toggling (optional, but good for performance)
    clearTimeout(scrollTimeout);
    scrollTimeout = setTimeout(() => {
        // Any delayed actions after scroll stops
    }, 100);
}
